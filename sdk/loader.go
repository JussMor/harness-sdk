...
// ═══════════════════════════════════════════════════════════════════════
// ParseSkillFile — parse a SKILL.md into a Skill
// ═══════════════════════════════════════════════════════════════════════

// ParseSkillFile reads a SKILL.md file and returns a fully populated Skill.
// The file MUST contain YAML frontmatter with at least: name, version,
// description, category, triggers. Everything after the frontmatter becomes
// the Skill.Content.
func ParseSkillFile(path string) (*Skill, error) {
	...
	return ParseSkillMarkdown(string(data))
}

// ParseSkillMarkdown parses a SKILL.md string (frontmatter + body) into a Skill.
func ParseSkillMarkdown(raw string) (*Skill, error) {
	...
	for _, req := range []string{"name", "version", "description", "category"} {
		if fields[req] == "" {
			return nil, fmt.Errorf("missing required skill field: %s", req)
		}
	}
	if len(lists["triggers"]) == 0 {
		return nil, fmt.Errorf("skill must have at least one trigger")
	}
	...
	meta := SkillMeta{
		...
		RequiredFeatureFlag: fields["requiredFeatureFlag"],
		GrantedTools:        lists["grantedTools"],
	}
	...
	return &Skill{
		...
		GrantedTools: meta.GrantedTools,
		Content:      body,
	}, nil
}

// LoadSkillsDir walks a directory and parses every SKILL.md found one level
// deep (i.e. skills/<name>/SKILL.md). Returns all successfully parsed skills.
